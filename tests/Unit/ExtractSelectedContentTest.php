...
it('extracts content when a selector matches one element', function () {
    $extractSelectedContent = new ExtractSelectedContent;
    $result = $extractSelectedContent->execute('<html><body><h1>Example Domain</h1></body></html>', 'h1');
...
it('extracts the first match when multiple elements match', function () {
    $extractSelectedContent = new ExtractSelectedContent;
    $result = $extractSelectedContent->execute('<html><body><h1>First</h1><h1>Second</h1></body></html>', 'h1');
...
it('throws when no selector matches are found', function () {
    ...
    expect(fn () => $extractSelectedContent->execute('<html><body><h1>Example</h1></body></html>', 'p'))
...
it('throws when selector syntax is invalid', function () {
    ...
    expect(fn () => $extractSelectedContent->execute('<html><body><h1>Example</h1></body></html>', 'h1['))
...
it('extracts nested html text content', function () {
    $extractSelectedContent = new ExtractSelectedContent;
    $result = $extractSelectedContent->execute('<html><body><h1>Hello <span>World</span></h1></body></html>', 'h1');
...
it('normalizes consecutive whitespace before hashing', function () {
    $extractSelectedContent = new ExtractSelectedContent;
    $result = $extractSelectedContent->execute("<html><body><h1>  Hello \n\t World   Again </h1></body></html>", 'h1');
...
it('hashes the empty string when normalized content is empty', function () {
    $extractSelectedContent = new ExtractSelectedContent;
    $result = $extractSelectedContent->execute('<html><body><div><span>   </span></div></body></html>', 'div');
...
it('parses recoverable malformed html when possible', function () {
    $extractSelectedContent = new ExtractSelectedContent;
    $result = $extractSelectedContent->execute('<html><body><div><h1>Example Domain', 'h1');
